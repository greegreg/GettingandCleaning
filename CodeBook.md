...
Each measured variable was defined by the original authors in two files 
...
Each column in DTA has a name. These names are given by the original authors of this study in the file "./features.txt". I will extract this information and use it below. A function is then created that will process the variable names, vn, variable and extract the location of the "mean()" and "std()" variables. The code used for this extraction was found on stack overflow's web site (wish I could claim it was mine).

The next step in the process is to create a binary vector identifying which names contain "mean()" or "std()". With this binary vector subset the original DTA data frame. The new data frame, DTA, contains only columns of data representing "mean()" and "std()" variables. 

From the vn data frame, where all of the variable names are stored, extract only the variable names containing "mean()" and "std()". Construct a vector from the data frame vnp. Extend this vector by adding "Person_ID" and "Activity_ID". Finally there is a vector with the names of all the variables of interest. At this point bind the paired data frame DTA with the DTARL data frame. This yields a data frame, DTA, with person ID and Activity ID as well as all measures containing "mean()" and "std()". Finally, place names for each variable into the DTA data frame using the vector vnames.
